...
while t < t_final:
    ...
    T_1 = np.array([[
        ...
        T(13)*c4 + T(9)*((4*tau)/3) + T(14)*((8*tau)/3)  + ((4*tau)/3)*((hgas*l*Tgas)/(k)),
    ...
    if steady and (np.max(np.abs(T_1-T_0))/dt) < 0.001:
        exportTemperatures(T_1, 'explicit_steady')
        t_steady = t
        print()
